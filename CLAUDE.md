...
- `maxNativeZoom` per layer is measured, not assumed: swisstopo grey/colour stop at 19, SWISSIMAGE and the Esri dark canvas at 20. The map keeps `maxZoom: 21` so Leaflet upscales rather than requesting tiles that 400
- **Do not re-add an `osm` layer.** OpenStreetMap's volunteer tile servers returned `403 Access blocked - App is not following the tile usage policy` to card users; their policy excludes app/bulk access and expects a descriptive `User-Agent`, which a browser cannot send. Blocking is per-IP, so it passes local testing and fails for real users. `RETIRED_BASEMAPS` keeps existing `basemap: osm` configs working (warn + fall back) instead of throwing
